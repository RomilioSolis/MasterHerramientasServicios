...
const SOCIAL_BUTTONS_HTML = `
<div class="social-buttons">
    <a href="https://www.facebook.com/masters.herramientas/" class="social-button facebook-button" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
        <svg viewBox="0 0 24 24" width="24" height="24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
    </a>
    <a href="https://www.instagram.com/masterenherramientasyservisios/" class="social-button instagram-button" target="_blank" rel="noopener noreferrer" aria-label="Instagram">
    ...
    <a href="https://wa.me/573165345675" class="social-button whatsapp-button" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp">
...
function loadStyles() {
    // Ya existe el estilo? No cargar dos veces
    if (document.getElementById('social-buttons-styles')) return;
    
    // Primero intentar cargar el CSS externo
    const link = document.createElement('link');
    link.id = 'social-buttons-styles';
    link.rel = 'stylesheet';
    ...
    // Timeout para fallback - si no carga en 2 segundos, usar estilos embebidos
    setTimeout(() => {
        if (!document.getElementById('social-buttons-styles')) injectInlineStyles();
    }, 2000);
}

function injectInlineStyles() {
    if (document.getElementById('social-buttons-inline-styles')) return;
    const style = document.createElement('style');
    style.id = 'social-buttons-inline-styles';
    style.textContent = SOCIAL_BUTTONS_CSS;
...
function loadHTML() {
    ...
    const tryLoad = () => {
        ...
        if (!container) {
            console.error('SocialButtons: container no encontrado después de', maxRetries, 'intentos');
            return;
        }
        
        console.log('SocialButtons: container encontrado, contenido:', container.innerHTML.length);
        if (container.innerHTML.trim()) return;
        
        container.innerHTML = SOCIAL_BUTTONS_HTML;
...
export default { 
    init: () => {
        try {
            console.log('SocialButtons.init(): iniciando...');
            loadStyles();
            loadHTML();
            // BackToTop también usa async, pero no requerimos esperar
            BackToTop.init().catch(e => console.error('BackToTop error:', e.message));
            console.log('SocialButtons.init(): completado');
        } catch (e) {
